...
import { ConfigurationsService, EventService, SubapplicationRespondService, WsEvents } from '@ws-widget/utils'
...
export class HtmlComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.activatedRoute.snapshot.queryParamMap.get('preview') === 'true' && !this.accessControlSvc.authoringConfig.newDesign) {
      ...
      this.viewerDataSubscription = this.viewerSvc
        ...
        .subscribe(async data => {
          data.artifactUrl = (
            data.artifactUrl.startsWith('https://')
              ? data.artifactUrl
                ...
                if (event.data.requestId && this.htmlData) {
                  switch (event.data.requestId) {
                    ...
                    case 'CONTINUE_LEARNING':
                      await this.respondSvc.continueLearningRespond(this.htmlData.identifier, event.data.data.continueLearning)
                      break
                    case 'TELEMETRY':
  ...
  private fireRealTimeProgress() {
    ...
    if (this.htmlData) {
      ...
      if (this.htmlData.resourceType && this.htmlData.resourceType.toLowerCase() === 'certification') {
        return
      }
    }
    ...
    this.viewerSvc.realTimeProgressUpdate(this.htmlData ? this.htmlData.identifier : '', this.realTimeProgressRequest)
    return
  }
}
