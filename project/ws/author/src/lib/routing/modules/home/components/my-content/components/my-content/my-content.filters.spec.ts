 ...
 */
describe('MyContentComponent (filters + failures)', () => {
  ...
  // This component's menu uses { name, values:[{ name, checked }] } — the lookup
  // matches on node.type for the group and node.displayName for the leaf, so the
  // fixture uses the same string for both.
    ...
    component.filterMenuItems = [
      {
        name: 'contentType',
        values: [
          { name: 'Course', checked: false },
          { name: 'Resource', checked: false },
        ],
      },
    ]
  ...
  describe('filterApplyEvent', () => {
    ...
    it('removes a filter that is unchecked', () => {
      ...
      component.filterMenuItems[0].values[0].checked = true
