...
/**
 * Shared behaviour of the content-detail and mandatory-content screens, which were
 * verbatim copies of one another. Subclasses supply the @Component metadata and
 * override the hooks below where the two genuinely differ.
 *
 * Carries `@Directive()` with no selector: subclasses inherit this constructor, and
 * Angular needs the decorator to resolve its dependencies (NG2006/NG2007). It is
 * abstract and never declared in an NgModule.
 */
@Directive()
export abstract class ContentDetailBaseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    protected dataService: LocalDataService,
    protected myTocService: MyTocService,
  ) {
  ...
  /**
   * content-detail reads the id from its route; mandatory-content receives it as an
   * @Input and must not overwrite it. Default is to leave it as the subclass set it.
   */
  protected resolveContentId(): void {}

  /** Extra fields mandatory-content adds to the glance card. */
  protected glanceExtras(): Record<string, any> {
    return {}
  }

  fetchContent() {
    this.resolveContentId()
    if (this.contentId) {
