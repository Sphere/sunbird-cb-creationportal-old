...
describe('FillUpsEditorComponent', () => {
  ...
  const fitbQuiz = (options: any[] = [{ text: 'apple' }, { text: 'banana' }]) => ({
    questionType: 'fitb',
  ...
  const load = (quiz: any = fitbQuiz(), index = 0) => {
  ...
  beforeEach(() => {
    ...
    quizStoreSvc = {
      selectedQuizIndex,
      getQuiz: jest.fn().mockReturnValue(fitbQuiz()),
    ...
    component = new FillUpsEditorComponent(new FormBuilder(), dialog, snackBar, quizStoreSvc, breakpointObserver)
  ...
  describe('ngOnInit', () => {
    ...
    it('builds the form for a fill-in-the-blanks question', () => {
      load()
      ...
      expect(options().at(0).value).toEqual({ text: 'apple', hint: '' })
    })

    it('ignores a question of another type', () => {
      load({ questionType: 'mcq-sca', options: [] })
  ...
  describe('assignForm', () => {
    it('marks the answer text dirty for an invalid question', () => {
      load()
      quizStoreSvc.getQuiz.mockReturnValue({ ...fitbQuiz(), isInValid: true })
  ...
  describe('addOption', () => {
    ...
    it('warns when there is no selected question', () => {
      load()
  ...
  describe('removeOption', () => {
    ...
    it('keeps the blank when the removal is cancelled', () => {
      load()
    ...
    it('tops the list back up to the minimum after a removal', () => {
      ...
      expect(component.selectedQuiz!.options.length).toBe(2)
      expect(component.selectedQuiz!.options[0].text).toBe('banana')
      expect(component.selectedQuiz!.options[1].text).toBe('')
    })

    it('just removes the blank when the list stays above the minimum', () => {
      load(fitbQuiz([1, 2, 3].map(n => ({ text: `t${n}` }))))
  ...
  describe('drop', () => {
    it('reorders the blanks', () => {
      load()

      component.drop({ previousIndex: 0, currentIndex: 1 } as any)

      expect(options().value.map((o: any) => o.text)).toEqual(['banana', 'apple'])
    })
  })
  ...
  describe('updateContentService', () => {
    it('writes the hint into the addressed blank', () => {
      load()

      component.updateContentService('<p>hint</p>', 1)

      expect(options().at(1).get('hint')!.value).toBe('<p>hint</p>')
    })

    it('ignores an out-of-range blank', () => {
      load()
  ...
  describe('form value changes', () => {
    it('emits the debounced form value with every blank marked correct', () => {
      const spy = jest.fn()
      component.value.subscribe(spy)
      load()

      options().at(0).get('text')!.setValue('cherry')
      jest.advanceTimersByTime(100)

      expect(spy).toHaveBeenCalledWith({
        options: [
          { text: 'cherry', hint: '', isCorrect: true },
          { text: 'banana', hint: '', isCorrect: true },
        ],
      })
    })
  })
