  ...
  private getParentUpdatedMeta(): NSContent.IContentMeta {
    ...
    Object.keys(this.authInitService.authConfig).map(v => {
      ...
      meta[v as keyof NSContent.IContentMeta] = parentMeta[v as keyof NSContent.IContentMeta]
        ...
        : this.authInitService.defaultValueFor(v as keyof IFormMeta, parentMeta.contentType)
    })
    return meta
  }
  ...
  checkCondition(id: string, meta: string, type: 'show' | 'required' | 'disabled'): boolean {
    ...
    try {
      ...
      if (
        ...
        this.authInitService.authConfig[meta as keyof IFormMeta][directType][data.contentType].map(condition => {
      ...
      if (
        ...
        this.authInitService.authConfig[meta as keyof IFormMeta][counterType][data.contentType].map(condition => {
          let childReturnValue = false
          Object.keys(condition).map(childMeta => {
            if (condition[childMeta as keyof typeof condition].indexOf(true) > -1 && this.isPresent(childMeta, id)) {
              childReturnValue = true
