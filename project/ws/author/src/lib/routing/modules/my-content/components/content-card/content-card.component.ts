...
export class ContentCardComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    if (!this.hideThumbnail) {
      // Publishing repoints appIcon at a 56px thumbnail generated by the
      // pipeline and keeps the uploaded image in posterImage, so a published
      // course renders blurred unless posterImage is preferred where present.
      this.data.appIcon = this.data.posterImage || this.data.appIcon || 'cbp-assets/icons/default.png'
    }
    if (
