...
export class PublicAboutComponent implements OnInit, OnDestroy {
  ...
  isSmallScreen$ = this.breakpointObserver.observe(Breakpoints.XSmall).pipe(map(breakPointState => breakPointState.matches))
  ...
  ngOnInit() {
    this.subscriptionAbout = this.activateRoute.data.subscribe(data => {
      ...
      if (this.aboutPage && this.aboutPage.banner && this.aboutPage.banner.videoLink) {
        this.videoLink = this.domSanitizer.bypassSecurityTrustResourceUrl(this.aboutPage.banner.videoLink)
      }
    })

    if (this.configSvc.instanceConfig) {
      this.headerBanner = `url('${this.configSvc.instanceConfig.logos.aboutHeader}')`
      this.footerBanner = `url('${this.configSvc.instanceConfig.logos.aboutFooter}')`
    }
  }
