...
These encode the _real_ conventions in this codebase (verified against actual files), so generated code matches what's already here instead of generic Angular boilerplate.
...
| [`release-notes`](release-notes/SKILL.md)           | Generating a production release note for a deploy candidate (diffs vs last `v*` tag, writes `RELEASE_NOTES/release-<X.Y.Z>.md`). |
...
2. Add frontmatter: `name` (matches the folder) and a `description` that states _what it does_ and _when to use it_ (include trigger phrases — this drives auto-selection).
